...
def test_llm_find_bug_add_function_incorrect_function(llm):
    ...
    function_code_with_unit_tests = FunctionCodeWithUnitTests(
        function_code, unit_tests, unit_test_imports
    )
    failing_test_message = FindFirstFailingUnitTestFunctionMessage(
        False, "test_add", "AssertionError: assert -1 == 5"
    )

    result = llm_find_bug(
        llm, query, function_code_with_unit_tests, failing_test_message
    )
...
def test_llm_find_bug_add_function_incorrect_unit_test(llm):
    ...
    failing_test_message = FindFirstFailingUnitTestFunctionMessage(
        False, "test_add", "AssertionError: assert 6 == 5"
    )
